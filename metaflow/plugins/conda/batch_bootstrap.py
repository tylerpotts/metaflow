...
def install_conda_environment(env_id, packages):
    args = [
        ...
        "echo starting conda create; start=$(date +%s)",
        "conda create --yes --offline --no-default-packages -p {0} --no-deps {1} >/dev/null 2>&1".format(
            os.path.join(os.getcwd(), env_id), " ".join(packages)
        ),
        "end=$(date +%s); echo seconds to completion; echo $((end-start))",
